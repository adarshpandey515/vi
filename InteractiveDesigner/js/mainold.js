...
async function convertChartsToStaticImagesForExport(exportContainer, editor) {
  if (!exportContainer || !editor) return;
  ...
  const exportCharts = Array.from(exportContainer.querySelectorAll(chartSelector));
  const liveCharts = Array.from(liveDoc.querySelectorAll(chartSelector));
  if (!exportCharts.length) return;

  for (let i = 0; i < exportCharts.length; i++) {
    const exportChart = exportCharts[i];
    const liveChart = exportChart.id
      ? liveDoc.getElementById(exportChart.id)
      : liveCharts[i];
    ...
    try {
      const liveCanvas =
        (liveChart.tagName && liveChart.tagName.toUpperCase() === "CANVAS")
          ? liveChart
          : liveChart.querySelector("canvas");
    ...
    if (!dataUrl) {
      try {
        const liveSvg =
          (liveChart.tagName && liveChart.tagName.toUpperCase() === "SVG")
            ? liveChart
            : liveChart.querySelector("svg");
        if (liveSvg) {
          const serializer = new XMLSerializer();
          const svgText = serializer.serializeToString(liveSvg);
    ...
    const rect = liveChart.getBoundingClientRect ? liveChart.getBoundingClientRect() : null;
    if (rect && rect.width > 0 && rect.height > 0) {
      img.style.width = `${Math.ceil(rect.width)}px`;
      img.style.height = `${Math.ceil(rect.height)}px`;
    }

    exportChart.replaceWith(img);
  }
}
...
(async function () {
  ...
  if (pageData) {
    ...
    scheduleDatasourceRebind("indexeddb-template-load", 250);
  }
})();
...
function importFile() {
  ...
  if (file) {
    ...
    reader.onload = function (e) {
      ...
      scheduleDatasourceRebind("import-single-page", 250);
      editor.Modal.close();
      const tempDiv = document.createElement('div');
...
function resolveTextContentFromDatasource(component, jsonPath, commonJson) {
  ...
  if (rawContent.includes("{") && rawContent.includes("}")) {
    ...
    jsonPaths.forEach((path) => {
      ...
      const placeholderToken = getPlaceholderTokenFromPath(commonJson, path);
      if (!placeholderToken) return;
